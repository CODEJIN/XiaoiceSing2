...
from Modules.Modules_Paper import XiaoiceSing2
...
class Trainer:
    ...
    def Train_Step(self, tokens, notes, durations, encoding_lengths, singers, genres, features, log_f0s, voices):
        ...
        loss = \
            self.hp.Train.Loss_Weight.Feature * loss_dict['Feature'] + \
            self.hp.Train.Loss_Weight.Log_F0 * loss_dict['Log_F0'] + \
            self.hp.Train.Loss_Weight.Voice * loss_dict['Voice']
        ...
        if self.steps >= self.hp.Train.Discrimination_Step:
            ...
            loss_dict['Feature_Matching'] = torch.stack([
            ...
            loss = loss + \
                self.hp.Train.Loss_Weight.Feature_Matching * loss_dict['Feature_Matching'] + \
                self.hp.Train.Loss_Weight.Adversarial * loss_dict['Adversarial']
    ...
    def Evaluation_Epoch(self):
        ...
        if self.gpu_id == 0:
            ...
            prediction_feature = predictions[index, :, :feature_length]

            log_f0_target = log_f0s[index, :feature_length].cpu().numpy()
            log_f0_prediction = log_f0_predictions[0, :feature_length].cpu().numpy()
