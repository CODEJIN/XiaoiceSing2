...
def Pattern_File_Generate(
    ...
    if mel.shape[0] > len(lyric):
        ...
        log_energy = log_energy[math.floor((log_energy.shape[0] - len(lyric)) / 2.0):-math.ceil((log_energy.shape[0] - len(lyric)) / 2.0)]
    elif len(lyric) > mel.shape[0]:
        print('Case2')
        fix_length = len(lyric) - mel.shape[0]
        lyric = lyric[0:-3 - fix_length] + lyric[-3:]
        note = note[0:-3 - fix_length] + note[-3:]        
