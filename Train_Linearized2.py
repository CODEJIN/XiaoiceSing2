...
from Datasets2 import Dataset, Inference_Dataset, Collater, Inference_Collater
...
class Trainer:
    ...
    def Train_Step(self, tokens, notes, lengths, singers, genres, features, log_f0s, voices):
        loss_dict = {}
    ...
    def Train_Epoch(self):
        for tokens, notes, lengths, singers, genres, features, log_f0s, voices in self.dataloader_dict['Train']:
            self.Train_Step(tokens, notes, lengths, singers, genres, features, log_f0s, voices)
    ...
    def Evaluation_Step(self, tokens, notes, lengths, singers, genres, features, log_f0s, voices):
        loss_dict = {}
        tokens = tokens.to(self.device, non_blocking=True)
        notes = notes.to(self.device, non_blocking=True)
        lengths = lengths.to(self.device, non_blocking=True)
    ...
    def Evaluation_Epoch(self):
        ...
        for step, (tokens, notes, lengths, singers, genres, features, log_f0s, voices) in tqdm(
            ...
            predictions, log_f0_predictions, voice_predictions = \
                self.Evaluation_Step(tokens, notes, lengths, singers, genres, features, log_f0s, voices)
        ...
        if self.gpu_id == 0:
            ...
            target_feature = features[index, :, :length]
            prediction_feature = predictions.detach()[index, :, :length]

            log_f0_target = log_f0s[index, :length].cpu().numpy()
            log_f0_prediction = log_f0_predictions.detach()[index, :length].cpu().numpy()
    ...
    def Inference_Step(self, tokens, notes, lengths, singers, genres, lyrics, start_index= 0, tag_step= False):
        tokens = tokens.to(self.device, non_blocking=True)
        notes = notes.to(self.device, non_blocking=True)
        lengths = lengths.to(self.device, non_blocking=True)
        genres = genres.to(self.device, non_blocking=True)
        ...
        for index, (prediction, log_f0, length, lyric, audio, file) in enumerate(zip(
            ...
            plt.imshow(prediction[:, :length], aspect='auto', origin='lower')
            plt.title('Prediction    {}'.format(title))
            plt.colorbar(ax= ax)
            ax = plt.subplot2grid((2, 1), (1, 0))
            plt.plot(log_f0[:length])
    ...
    def Inference_Epoch(self):
        ...
        batch_size = self.hp.Inference_Batch_Size or self.hp.Train.Batch_Size
        for step, (tokens, notes, lengths, singers, genres, lyrics) in tqdm(
            enumerate(self.dataloader_dict['Inference']),
            desc='[Inference]',
            total= math.ceil(len(self.dataloader_dict['Inference'].dataset) / batch_size)
            ):
            self.Inference_Step(tokens, notes, lengths, singers, genres, lyrics, start_index= step * batch_size)
